...
describe('Linux AppImage Media Framework and v0.1.9 Consistency', () => {
	it('R1: electron-builder.json enables AppImage for Linux and matches v0.1.9', () => {
	...
	it('R2: All Cargo crate manifests and package.json are bumped to 0.1.9', () => {
		const pkg = JSON.parse(fs.readFileSync(path.join(rootDir, 'package.json'), 'utf-8'));
		expect(pkg.version).toBe('0.1.9');
		...
		for (const file of cargoFiles) {
			...
			expect(content).toMatch(/version\s*=\s*"0\.1\.9"/);
		}
	});

	it('R2 & R3: UI about page, updater service, and sync script reference 0.1.9', () => {
		...
		expect(aboutContent).toContain('v0.1.9');
		...
		expect(updaterContent).toContain("const currentVersion = '0.1.9'");
		...
		expect(syncContent).toContain('v0.1.9');
	});
});
