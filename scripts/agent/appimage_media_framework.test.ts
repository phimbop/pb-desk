...
describe('Linux AppImage Media Framework and v0.1.6 Consistency', () => {
	it('R1: tauri.conf.json enables bundleMediaFramework for Linux AppImage', () => {
		...
		expect(conf.version).toBe('0.1.6');
	});
	...
	it('R2: All Cargo crate manifests and package.json are bumped to 0.1.6', () => {
		...
		for (const file of cargoFiles) {
			...
			expect(content).toMatch(/version\s*=\s*"0\.1\.6"/);
		}
	});

	it('R2 & R3: UI about page, updater service, and sync script reference 0.1.6', () => {
		...
		expect(aboutContent).toContain('v0.1.6');
		...
		expect(updaterContent).toContain("const currentVersion = '0.1.6'");
		...
		expect(syncContent).toContain('v0.1.6');
	});
});
