<script lang="ts">
	...
	const loadMovies = async (p: number) => {
		...
		error = null;
		try {
			const res = await api.getMoviesByCategory(slug, p, 20);
			moviesData = res;
	...
	let genreName = $derived(
		getLocale() === 'vi'
			? (kkMovieGenres.find((g) => g.slug === slug)?.name || slug?.replace(/-/g, ' ') || '')
			: (kkMovieGenresEnglish.find((g) => g.slug === slug)?.name || slug?.replace(/-/g, ' ') || '')
	);
</script>
