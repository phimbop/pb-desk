<script lang="ts">
	...
	const loadMovies = async (p: number) => {
		...
		error = null;
		try {
			const res = await api.getMoviesByCountry(slug, p, 20);
			moviesData = res;
			window.scrollTo({ top: 0, behavior: 'smooth' });
		} catch (err: any) {
			console.error('Failed to load country movies:', err);
			error = err?.message || 'Không thể tải danh sách phim theo quốc gia.';
		} finally {
			loading = false;
		}
	};
	...
	let countryName = $derived(slug ? slug.replace(/-/g, ' ') : '');
</script>
