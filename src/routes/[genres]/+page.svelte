<script lang="ts">
	...
	const loadMovies = async (p: number) => {
		...
		error = null;
		try {
			const res = await api.getMoviesByCategory(genreSlug, p, 20);
			moviesData = res;
			window.scrollTo({ top: 0, behavior: 'smooth' });
		} catch (err: any) {
			console.error('Failed to load category movies:', err);
			error = err?.message || 'Không thể tải danh sách phim theo thể loại.';
		} finally {
			loading = false;
		}
	};
	...
	let genreName = $derived(
		getLocale() === 'vi'
			? (kkMovieGenres.find((g) => g.slug === genreSlug)?.name || genreSlug?.replace(/-/g, ' ') || '')
			: (kkMovieGenresEnglish.find((g) => g.slug === genreSlug)?.name || genreSlug?.replace(/-/g, ' ') || '')
	);
</script>
...
{:else if moviesData && moviesData.items.length > 0}
	<section class="w-full mb-20 sm:mb-0 px-2 sm:px-0 sm:mt-20 relative">
		...
		<a href={page.url.pathname}>
			<h1 class="text-2xl font-bold text-slate-200 pl-2 mt-30 md:mt-0 md:pl-5 mb-10 capitalize">
				{m.reuse_list()} {genreName}
			</h1>
		</a>
