...
{#key page.url}
	<section class="w-full min-h-screen flex relative" transition:fade={{ duration: 150 }}>
		<div
			use:clickOutside={() => {
		...
		>
			...
			{#if kkPhimboDetail?.episodes?.[0]?.server_data}
				{#each kkPhimboDetail.episodes[0].server_data as episodes}
					<a
						href="/phim-bo/{kkPhimboDetail.movie.slug}/{episodes.slug}"
						onclick={() => {
							if (playedListStore.value.some((item) => item._id === kkPhimboDetail.movie?._id)) {
								const index = playedListStore.value.findIndex(
									(item) => item._id === kkPhimboDetail.movie._id
								);
								if (index !== -1) {
									playedListStore.value = playedListStore.value.with(index, {
										...playedListStore.value[index],
										playedTime: 0
									});
							...
							if (
								kkPhimboTapPhimStore.value.some((item) => item.movie === kkPhimboDetail.movie.slug)
							) {
								...
								kkPhimboTapPhimStore.value = [
									...kkPhimboTapPhimStore.value,
									{ movie: kkPhimboDetail.movie.slug, episodes: episodes.slug }
								];
							}
						}}
						class="relative flex items-center {checkMovieEpisodes(
							kkPhimboDetail.movie.slug,
							episodes.slug
						)
							? 'text-neonPink-500'
		...
		<div
			class="{sidebarOpen.value
				? 'translate-x-3/4 -ml-[75%] md:ml-0 md:w-5/6 md:translate-x-0'
		...
		>
			...
			>
				...
				<a href="/phim-bo" class="line-clamp-1"> {m.reuse_series()}</a>
				{#if kkPhimboDetail?.movie}
					<span class="mx-2">⇀</span>
					<a class="line-clamp-1" href="/phim-bo/{kkPhimboDetail.movie.slug}"
						>{getLocale() === 'vi' ? kkPhimboDetail.movie.name : kkPhimboDetail.movie.origin_name}</a
					>
					{#if page.url.pathname !== `/phim-bo/${kkPhimboDetail.movie.slug}`}
						<span class="mx-2">⇀</span>
