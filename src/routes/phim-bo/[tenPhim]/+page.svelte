<script lang="ts">
	...
	import { DOMAIN_KKPHIM_IMAGE, patternKKImageDomain, websiteUrl, getKKImageUrl } from '$lib';
	...
	const kkPhimboDetail: kkPhimboDetailType = $derived(page.data.kkPhimboDetail);
	let currentEpisode = $derived(
		kkPhimboTapPhimStore.value.find((item) => item.movie === kkPhimboDetail?.movie?.slug)?.episodes
	);
	let movieListed = $derived(
		myListMoviesStore.value.some((item) => 
			(item._id && kkPhimboDetail?.movie?._id && item._id === kkPhimboDetail.movie._id) ||
			(item.slug && kkPhimboDetail?.movie?.slug && item.slug === kkPhimboDetail.movie.slug)
		)
	);
	let playedTime = $derived(
		playedListStore.value.find((item) => item._id === kkPhimboDetail?.movie?._id)?.playedTime ?? 0
	);
	let duration = $derived(
		playedListStore.value.find((item) => item._id === kkPhimboDetail?.movie?._id)?.duration ?? 0
	);
...
{#if kkPhimboDetail?.movie}
	<Seo
		title={kkPhimboDetail.movie.name +
			' | ' +
			kkPhimboDetail.movie.origin_name +
			' (' +
			(kkPhimboDetail.movie.year?.toString() ?? '') +
			') 4K FULL HD 1080p Vietsub Online miễn phí'}
		metadescription={kkPhimboDetail.movie.content}
		slug={`${page.url}`}
		image={`${kkPhimboDetail.movie.poster_url}`}
		keywordsPlus={kkPhimboDetail.movie.name}
		jsonLd={{
	...
	<div class="relative w-full h-fit sm:h-screen overflow-hidden">
		...
		<div class="relative sm:absolute w-full h-fit sm:h-screen inset-0">
			<a
				aria-label="Play phim bộ"
				href="/phim-bo/{kkPhimboDetail.movie.slug}/{currentEpisode
					? currentEpisode
					: (kkPhimboDetail.episodes?.[0]?.server_data?.[0]?.slug ?? 'tap-1')}"
				onclick={() => {
					...
					if (kkPhimboTapPhimStore.value.some((item) => item.movie === kkPhimboDetail.movie.slug)) {
						kkPhimboTapPhimStore.value = kkPhimboTapPhimStore.value.map((item) => {
							if (item.movie === kkPhimboDetail.movie.slug && !currentEpisode) {
								return { ...item, episodes: kkPhimboDetail.episodes?.[0]?.server_data?.[0]?.slug ?? 'tap-1' };
							} else {
								return item;
							}
						});
						...
						kkPhimboTapPhimStore.value = [
							...kkPhimboTapPhimStore.value,
							{
								...
								episodes: kkPhimboDetail.episodes?.[0]?.server_data?.[0]?.slug ?? 'tap-1'
							}
						];
					}
				}}
			...
			>
				<Image
					src={getKKImageUrl(kkPhimboDetail.movie.thumb_url)}
					alt={kkPhimboDetail.movie.name}
					isCritical={true}
		...
		>
			...
			>
				{getLocale() === 'vi' ? kkPhimboDetail.movie.name : ''} ({kkPhimboDetail.movie.origin_name})
			</h1>
			<div
			...
			>
				...
				>
					<p class="text-slate-400">
						{m.reuse_year()}: <span class="text-slate-200">{kkPhimboDetail.movie.year}</span>
					</p>
					<p class="text-slate-400">
						{m.reuse_runtime()}: <span class="text-slate-200">{kkPhimboDetail.movie.time}</span>
					</p>
					<p class="text-slate-400">
						{m.reuse_status()}:
						<span class="text-slate-200">{kkPhimboDetail.movie.episode_current}</span>
					</p>
			...
			<div class="group flex flex-col lg:justify-end relative">
				...
				>
					<p class="text-slate-400">
						{m.reuse_directors()}: <span class="text-slate-200">{(kkPhimboDetail.movie.director || []).join(', ')}</span>
					</p>
					<p class="text-slate-400">
						{m.reuse_actors()}: <span class="text-slate-200">{(kkPhimboDetail.movie.actor || []).join(', ')}</span>
					</p>
					<p class="text-slate-400 my-4">
						{m.reuse_genres()}:
						{#each kkPhimboDetail.movie.category || [] as category}
					...
					<p class="text-slate-400">
						{m.reuse_overview()}: <span class="text-slate-200">{kkPhimboDetail.movie.content}</span>
					</p>
				</div>
				<p
				...
				<div class="mt-4 mb-4">
					<StarRating
						movieId={kkPhimboDetail.movie._id}
						user={page.data.user}
				...
				<div class="flex flex-wrap gap-3 mt-4">
					{#if myListMoviesStore.value}
						<button
							...
							onclick={async (e) => {
								e.preventDefault();
								await moviesHandler.updateMyListMovies(kkPhimboDetail.movie, 'kk');
							}}
						>
							{#if movieListed}
	...
	{#if page.data.user}
		<AddToPlaylistModal
			...
			movieId={kkPhimboDetail.movie._id}
			title={kkPhimboDetail.movie.name}
			posterPath={kkPhimboDetail.movie.poster_url}
			isTv={true}
		/>
	{/if}
	<section
	...
	>
		{#if kkPhimboDetail.movie.category?.[0]?.slug}
			<SectionMovieRelate
				slug={kkPhimboDetail.movie.category[0].slug}
