<script lang="ts">
	...
	let currentPageStore = useLocalStorage<number | undefined>('currentPageBo', 1);
	...
	const loadMovies = async (page: number) => {
		...
		error = null;
		try {
			const res = await api.getMoviesByType('phim-bo', page, 20);
			moviesData = res;
			currentPageStore.value = page;
			window.scrollTo({ top: 0, behavior: 'smooth' });
		} catch (err: any) {
			console.error('Failed to load series:', err);
			error = err?.message || 'Không thể tải danh sách phim bộ.';
		} finally {
			loading = false;
		}
	};
...
{:else if moviesData && moviesData.items.length > 0}
	<section class="w-full sm:mt-20 px-2 sm:px-0 relative">
		{#if loading}
			<LoadingSubBlur />
		{/if}
		<div class="flex mx-auto">
			<div class="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-10 gap-2 w-full h-full">
				{#each moviesData.items as movie (movie.id || movie.slug)}
					<CardkkPhimLe {movie} type="phim-bo" showDate={true} class="" />
				{/each}
			</div>
		</div>
