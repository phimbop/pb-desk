<script lang="ts">
	...
	let currentPageStore = useLocalStorage<number | undefined>('currentPageTinhCam', 1);
	...
	const loadMovies = async (page: number) => {
		...
		error = null;
		try {
			const res = await api.getMoviesByCategory('tinh-cam', page, 20);
			moviesData = res;
			currentPageStore.value = page;
			window.scrollTo({ top: 0, behavior: 'smooth' });
		} catch (err: any) {
			console.error('Failed to load romance movies:', err);
			error = err?.message || 'Không thể tải danh sách phim tình cảm.';
		} finally {
			loading = false;
		}
	};
