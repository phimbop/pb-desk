<script lang="ts">
	...
	const kkPhimboDetail: kkPhimboDetailType = $derived(page.data.kkPhimHoatHinhDetail);
	const hideBtnPlayNext = $derived(kkPhimboDetail?.episodes?.[0]?.server_data?.length == 1);
	...
	const playNext = () => {
		...
		if (index < serverData.length - 1) {
			...
			goto(`/phim-hoat-hinh/${kkPhimboDetail.movie.slug}/${nextEp}`);
		} else {
			...
			if (firstEp) {
				...
				goto(`/phim-hoat-hinh/${kkPhimboDetail.movie.slug}/${firstEp}`);
			}
		}
	};
...
{#if kkPhimboDetail?.movie && currentEpData}
	<Seo
		...
		jsonLd={{
			...
			description: kkPhimboDetail.movie.content,
			director: {
				'@type': 'Person',
	...
	>
		...
		{@html serializeSchema({
			...
			"itemListElement": [{
				...
				"@type": "ListItem",
				"position": 2,
				"name": `${kkPhimboDetail.movie.name} | ${kkPhimboDetail.movie.origin_name} ${kkPhimboDetail.movie.year?.toString() ?? ''}`,
				"item": `${websiteUrl}/phim-hoat-hinh/${kkPhimboDetail.movie.slug}`
			},{
				"@type": "ListItem",
				"position": 3,
			...
			<div class="w-full aspect-video relative md:h-screen md:overflow-hidden">
				{#if browser && playedListStore.state === 'ready' && KkPlayer}
					<KkPlayer
						...
						bind:videoEnded
						bind:playMovieInfo
						HideBtnPlayNext={hideBtnPlayNext}
					/>
				{/if}
			</div>
		</div>
