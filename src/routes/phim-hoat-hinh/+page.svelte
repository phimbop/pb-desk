<script lang="ts">
	...
	let currentPageStore = useLocalStorage<number | undefined>('currentPageHoatHinh', 1);
	...
	const loadMovies = async (page: number) => {
		...
		error = null;
		try {
			const res = await api.getMoviesByType('hoat-hinh', page, 20);
			moviesData = res;
			currentPageStore.value = page;
			window.scrollTo({ top: 0, behavior: 'smooth' });
		} catch (err: any) {
			console.error('Failed to load animation movies:', err);
			error = err?.message || 'Không thể tải danh sách phim hoạt hình.';
		} finally {
			loading = false;
		}
	};
		...
		<div class="flex mx-auto">
			<div class="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-10 gap-2 w-full h-full">
				{#each moviesData.items as movie (movie.id || movie.slug)}
					<CardkkPhimLe {movie} type="phim-hoat-hinh" showDate={true} class="" />
				{/each}
			</div>
		</div>
