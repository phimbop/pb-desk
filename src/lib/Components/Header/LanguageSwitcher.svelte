...
<div class="relative inline-block text-left lang-switcher z-[100]">
	...
	{#if isOpen}
		<div
			class="absolute right-0 bottom-12 z-50 w-40 origin-bottom-right rounded-lg bg-slate-800 border border-slate-700 p-1 shadow-xl focus:outline-hidden"
			role="menu"
			aria-orientation="vertical"
		>
			{#each locales as locale}
				<button
					class="w-full rounded-md text-left block px-3 py-1.5 text-xs text-slate-300 transition-colors hover:bg-slate-700 hover:text-white {locale === currentLocale ? 'bg-slate-700/50 text-neonPink-500 font-bold' : ''} cursor-pointer"
					role="menuitem"
