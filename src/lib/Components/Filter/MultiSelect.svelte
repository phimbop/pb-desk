<script lang="ts">
	...
	function selectOption(option: any) {
		if (selectedOptions.some((item) => item?.toString() === option?.toString())) {
			selectedOptions = selectedOptions.filter((item) => item?.toString() !== option?.toString());
		} else {
				...
				{#each options as option}
					...
					>
						<input
							...
							checked={selectedOptions.some((item) => item?.toString() === option.id?.toString())}
						/>
						<p class="peer-checked/filter-checkbox:text-neonPink-500 transition duration-200 select-none">
							{option.name}
						</p>
					</li>
				{/each}
